...
# ==================== 文件大小限制（业务规则） ====================
# ⚠️ 已废弃：请使用 config.get_config().max_audio_size 从YAML配置文件读取
# 保留此常量仅用于向后兼容，新代码应使用Config类

MAX_FILE_SIZE_MB = 500  # 最大文件大小（MB，已废弃，请使用Config）
MAX_FILE_SIZE_BYTES = MAX_FILE_SIZE_MB * 1024 * 1024  # 最大文件大小（字节，已废弃，请使用Config）
